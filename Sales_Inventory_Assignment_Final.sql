...
-- CREATE Customers table , Products table & Sales table in the assignment schema
...
select product_id, product_name, avg(price) as avg_price
from products p
group by (product_id);
...
select customer_id, concat(first_name,'-',last_name) as full_name, membership_status
from assignment.customers c ;

alter table assignment.customers add column full_name  varchar (50)

alter table assignment.customers drop column Full_name;
...
select product_id, product_name,supplier,category
from assignment.products p 
where category = 'Electronics';

-- 9. Write a query to find the highest price from the `Products` table.

select max(price) as highest_price from assignment.products p ;
...
select count(s.quantity_sold) as Total_quantity_sold
from assignment.sales s ;
...
select c.customer_id, c.first_name, c.last_name, p.product_id, p.price
from assignment.customers c 
...
select c.first_name, c.last_name, product_name, s.quantity_sold
from assignment.customers c 
inner join assignment.sales s 
on c.customer_id = s.customer_id 
inner join assignment.products p 
...
---- use the concat operator || " " || to separate the first name and last name and pair  wiyh another column

select c.customer_id as customer1_id, 
	   c.first_name || ' ' || c.last_name as Customer_pair,
	   c2.customer_id as customer2_id,
	   c2.first_name || ' ' ||c2.last_name as Customer_pair,
	   c.membership_status 
from assignment.customers c 
join assignment.customers c2
on c.membership_status  = c2.membership_status 
and c.customer_id < c2.customer_id; --(this line ensures no duplicates or mismatch)
...
select c.first_name, c.last_name, p.product_id, p.category 
from assignment.products p 
inner join assignment.sales s
on p.product_id = s.product_id 
...
select s.product_id , p.product_name, sum(s.total_amount ) as total_sales_amount
from assignment.sales s
inner join assignment.products p 
on s.product_id = p.product_id 
...
select p.product_id, p.product_name, max(p.price)
from assignment.products p
join assignment.sales s 
on p.product_id = s.product_id 
...
select count(*) as spend_over_500
from assignment.sales s 
where (s.total_amount) > 500;


select s.customer_id, c.first_name, s.total_amount 
from assignment.sales s
join assignment.customers c 
on c.customer_id = s.customer_id 
where (s.total_amount )>500;
...
select sum (s.total_amount ) as total_no_of_sales, c.membership_status  
from assignment.products p 
join assignment.sales s
on s.product_id = p.product_id 
join assignment.customers c 
...
select c.customer_id, c.first_name, sum(s.quantity_sold) as total_quantiy
from assignment.customers c 
inner join assignment.sales s 
on c.customer_id = s.customer_id 
...
select p.product_id, p.product_name, avg (s.quantity_sold) as average_quantity
from assignment.products p
join assignment.sales s
on s.product_id  = p.product_id 
...
select 
sum(s.total_amount) as December23_sales_amount, 
count (s.sale_id ) as total_transactions_December23
from assignment.sales s 
where s.sale_date between '2023-01-01' and '2023-12-31';
...
select c.customer_id, c.first_name, c.last_name, sum(s.total_amount) as total_spend, s.sale_date 
from assignment.customers c 
...
where (c.registration_date - s.sale_date >=7) 
group by c.customer_id , c.first_name , c.last_name, s.sale_date;
...
create index idx_customer_email on assignment.customers(email);
...
count (s.sale_id ) as customer_frequency
from assignment.sales s
...
sum(s.total_amount) as total_amount_spend
from assignment.customers c 
...
join assignment.products p  on i.product_id = p.product_id;
...
order by sale_month, sale_year desc;
...
select c.customer_id, c.first_name, c.last_name, sum(s.total_amount) as total_purchase
from assignment.sales s 
join assignment.customers c on s.customer_id   = c.customer_id
group by c.customer_id , c.first_name 
...
select distinct p.product_name, s.sale_date,
sum(s.quantity_sold) as total_products_sold
from assignment.products p 
join assignment.sales s on s.product_id = p.product_id 
where s.sale_date between '2023-01-01' and '2023-12-31'
group by p.product_name, s.sale_date ;
...
from assignment.customers c 
join assignment.sales s on c.customer_id = s.customer_id 
group by c.customer_id , c.first_name, s.total_amount 
order by s.total_amount desc;
...
from assignment.products p 
join assignment.sales s on p.product_id = s.product_id 
group by p.product_id , p.product_name, s.sale_id  
